"""
generate_vhdl_roms.py — Deterministic VHDL ROM generator for KingWen9BitResolver.

Single source of truth: kingwen_ternary_tables_complete.py :: HEXAGRAM_BASE.
Emits corrected UPPER_IDX_ROM, LOWER_IDX_ROM, ACTION_ROM constants into
src/hardware/KingWen9BitResolver.vhd. No hand-transcription. No RNG.

Run from repo root:
    python3 scripts/generate_vhdl_roms.py
"""
...
UPPER = [HB[h]["upper_idx"] for h in range(1, 65)]
LOWER = [HB[h]["lower_idx"] for h in range(1, 65)]
ACTION_CODE = {"ASSERT": "00", "YIELD": "01", "ADAPT": "10", "WAIT": "11"}
ACTIONS = [ACTION_CODE[HB[h]["action"]] for h in range(1, 65)]


def fmt_int_rom(vals):
    rows = []
...
def fmt_action_rom(vals):
    rows = []
    for i in range(0, 64, 8):
        rows.append("    " + ", ".join('"%s"' % v for v in vals[i : i + 8]))
    return ",\n".join(rows)


VHDL = f"""-- King Wen 9-Bit State Resolver (POG3 Sovereign Stack)
...
OUT = os.path.join(REPO, "src", "hardware", "KingWen9BitResolver.vhd")
with open(OUT, "w") as f:
    f.write(VHDL)
print("WROTE", OUT)
print("upper_rom[1..16]:", UPPER[:16])
print("lower_rom[1..16]:", LOWER[:16])
print("action_rom sample hex1/hex26/hex52/hex56:", ACTIONS[0], ACTIONS[25], ACTIONS[51], ACTIONS[55])
