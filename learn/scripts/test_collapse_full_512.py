...
def main() -> int:
    ...
    _result = shotgun_expand(emotional_input=args.emotional_input)
    expanded = _result.get("expanded") or []
    resolved = _result.get("resolved") or []
    from emotional_engine import _compute_consensus_from_resolved
    consensus = _compute_consensus_from_resolved(resolved, args.emotional_input)
    ...
    consensus_hex = consensus.get("consensus_hexagram_id")
    # consensus_hexagram_id is None when no single hex collapse (shotgun_expand design)
    assert consensus_hex is None or consensus_hex in range(1, 65), f"invalid consensus hexagram: {consensus_hex}"
    ...
    assert consensus.get("consensus_temporal") in (
        ...
        "resolution", "void", "transition", "dissolution", "crystallization", None
