...
import { EmotionalVector, TemporalState, HexagramAction, HexagramCategory } from '../types/oracle.js';
import { IntentVector, emotionalToIntent, intentToBits } from '../types/IntentVector.js';
import { CollapsedHexagramState, StateCapture, StateTelemetry, captureToTelemetry } from '../types/StateCapture.js';
...
export class HexagramRuntimeBridge {
  ...
  private stateHistory: StateCapture[];
  private hexagramRegistry: Record<string, Record<string, string>>;
  private emotionalWeights: Record<string, Record<string, number>>;

  constructor(
    sessionId: string,
    hexagramRegistry?: Record<string, Record<string, string>>,
    emotionalWeights?: Record<string, Record<string, number>>,
  ) {
   ...
   */
  consult(intent: IntentVector, context?: Record<string, unknown>): [CollapsedHexagramState, StateCapture] {
    this.tickCounter += 1;
    ...
    const collapsedState: CollapsedHexagramState = {
      state_id: bits,
      yao_lines: yaoLines,
      ...
      provenance: {
        ...
        king_wen_action: (registryEntry.action ?? 'WAIT') as HexagramAction,
        king_wen_category: (registryEntry.category ?? 'transformer') as HexagramCategory,
      },
    };
  ...
  ): [CollapsedHexagramState, StateCapture] {
